...
<nav class="w3-sidebar w3-collapse w3-white w3-animate-left" style="z-index:3;width:300px;" id="mySidebar"><br>
  ...
  <div class="w3-bar-block">
    ...
    <a href="#" class="w3-bar-item w3-button w3-padding w3-text-black"> Photographer</a>
...
<table class="table table-bordered table-hover">
  <thead class="thead-dark">
    <tr>
      ...
      <th scope="col">experience_time Id</th>
      <th scope="col">digital_post</th>
  ...
  <tbody>
    <tr v-for="values in articleDetail" :key="values.id">
      <td>{{values.local_id}}</td>
      <td>{{values.Fullname}}</td>
      <td>{{values.academic_year}}</td>
      <td>{{values.article_name}}</td>
      <td>{{values.experience_time}}</td>
      <td>{{values.digital_post}}</td>
      <td>{{values.email}}</td>
...
export default{
  ...
  mounted(){
      ...
      axios
      .get("http://127.0.0.1:8000/api/getArticleDetail")
        .then((response)=>{
  ...
  methods:{
    ...
    send(){
      ...
      axios 
        .post("http://127.0.0.1:8000/api/sendArticleId",this.form)
        .then((response)=>{
          if(response.status =='200'){
