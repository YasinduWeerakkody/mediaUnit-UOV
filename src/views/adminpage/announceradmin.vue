...
<nav class="w3-sidebar w3-collapse w3-white w3-animate-left" style="z-index:3;width:300px;" id="mySidebar"><br>
  ...
  <div class="w3-bar-block">
    ...
    <a href="/adminpage" class="w3-bar-item w3-button w3-padding"><i class="fa fa-eye fa-fw"></i>  photographer</a>
    <a href="/articleadmin" class="w3-bar-item w3-button w3-padding"><i class="fa fa-users fa-fw"></i>  Article</a>
...
<div class="w3-main" style="margin-left:300px;margin-top:43px;">
  ...
  <div class="form-outline mamm">
      <input type="text" id="form3Examplev3"
          v-model="form.AdminCreateID"
          class=" form-control-lg" placeholder="Create ID" />
   
  </div>
  <div class="mamm ">
<button class="w3-button w3-black" @click="save()">Create ID</button>
</div>
  </div>
...
<table class="table table-bordered table-hover">
  ...
  <tbody>
    <tr v-for="values in announcerDetail" :key="values.id">
      ...
      <td><button @click="select(values)">Confirm</button></td>
    </tr>
...
export default{
  ...
  mounted(){
      ...
      axios
      .get("http://127.0.0.1:8000/api/getAnnouncerDetails")
      .then((response)=>{
        if(response.status == '200'){
          // alert(response);
            this.announcerDetail=response.data.data;
        }
      })
  ...
  methods:{
    select(values){
        ...
        this.form.announcingID=values.id;
        this.form.memberID=values.local_id;
        this.form.Fullname=values.Fullname;
    },

    save(){
      axios
      .post("http://127.0.0.1:8000/api/anouncingSave",this.form)
      .then((response)=>{
        if(response.status=='200'){
